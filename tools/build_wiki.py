...
from wiki_data import CATEGORY_PAGES, DataError, PAGE_FILES, load_data, validate_data
...
def build_pages(root, data):
    validate_data(data)
    pages = {}
    for title, filename in PAGE_FILES.items():
...
def main(argv=None):
    ...
    try:
        ...
        return 1
    print(f"Wrote {len(pages)} pages; omitted {len(PAGE_FILES) + 1 - len(pages)} existing titles.")
